...
This plugin is a good place to get started when building a UXP plugin using React. It comes defined with all the dependencies that you'll need to get started. As this is a React project, you'll need to do some initial configuration before this will be usable in the host application.
...
1. Make sure a compatible host application is running and you can see it under 'Connected apps'
...
3. From the plugin's action menu •••
   - Configure the `/dist` folder by using `More` -> `Options` -> `Advanced`.
   - Select `Load` to view the plugin inside your application.
   - (Optional) Select `Load & Watch` to reload the plugin automatically every time the code in `/dist` changes.
